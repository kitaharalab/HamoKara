...
public class HamoKara extends PApplet {
	...
	public void keyPressed() {
		...
		if(isMenu) {
			...
			if (keyCode == ENTER) {
				...
				//}else if(musicnum==11){
				    midi = cmx.readSMFAsMIDIXML(createInput("../midis/X54963G2.MID"));//366日
				}


				//精度
				if(checking){
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/03.mid"));
				}
				...
				try {
					...
					mainNote = new long[notelist.length];     //
					mainOnset = new long[notelist.length];         //
					mainOffset = new long[notelist.length];          //
					hamoNote = new long[notelist.length];           //
					musicLength=notelist.length;

					

					for (int i=0; i<notelist.length; i++) {
						mainNote[i]=notelist[i].notenum();
						mainOnset[i]=notelist[i].onset();
						mainOffset[i]=notelist[i].offset();
			    	}
					...
					*/
					...
					if(isAccompany){
						...
						for (int i = 0; i < parts.length; i++) {
							if (parts[i].channel() != 1 && parts[i].channel()!=15) {
								part_all = parts[i];

								if (part == null) {
									throw new IllegalStateException("no part with channel=1");
								} 				   
								events = part_all.getNoteList();     
								for (MutableMusicEvent evt : events) {
									if (evt instanceof MutableProgramChange) {
										//((MutableProgramChange)evt).setValue(4);       //音色変更
									} else if (evt instanceof MutableControlChange) {
					...
					if(checking){
						for (int i=0; i<mainOnset.length; i++) {
							for (int j=0;j<onsetCheck.length; j++){
								if (mainOnset[i] == onsetCheck[j]/2  & mainOffset[i] == offsetCheck[j]/2 & hamoNote[i] == notenumCheck[j]){
									match++;
								}
							}
						...
						try{
							...
							printwriter.println("match="+ match + "per=" + match/musicLength); 
							printwriter.close();
						}catch(IOException e){
	...
	class Karaoke {
		...
		int toLyricP=0;
		//int j =0;
        int now_piano_hikisu=0;
		double now_piano_noteunm=-100.0;
		double user_notenum=-100.0;
		double hazure_check_count=0;
		double total_count=0;
		double seikai_check_count=0;
		double seikairitsu=100.0;
		double notenum_difference=0.0;    
				...
				if (lyriccount+i < lyrics.size()) {
					...
					if(checking){  
						for(int p=0;p<onsetCheck.length;p++){
							...
							//精度チェック
							stroke(255,0,0);
							line(((onsetCheck[p]/2 - div * (q-1))*width/div),(linemid * (lineNum - (notenumCheck[p] - subst)-1)),
									((mainOffset[p]/2 -div*(q-1))*width/div),(linemid * (lineNum - (notenumCheck[p] - subst))));
						}
					}
                    ...
                    // draw F0
					for (int p = 1; p < HamoKara.dioM.ticks.size(); p++) {
						double x0 = (HamoKara.dioM.ticks.get(p-1) - div * (q-1)) * width / div;
						double x1 = (HamoKara.dioM.ticks.get(p) - div * (q-1)) * width / div;
						double y0 = linemid * (lineNum - ((HamoKara.dioM.pitches.get(p-1) + octave*12) - subst)-2);
						double y1 = linemid * (lineNum - ((HamoKara.dioM.pitches.get(p) + octave*12) - subst)-2);
						if (Double.isFinite(y0) && Double.isFinite(y1)) {
							if(abs((int)y0-(int)y1) < 12){  
								strokeWeight(5);
								line((int)x0-80 , (int)y0, (int)x1-80 , (int)y1);
								strokeWeight(1);
							}  
						}
					}
	...
	class Menu {
		...
		void displayText() {
			...
			if( abs(mouseX-100)<=40 & abs(mouseY-540)<=17){
				rect(100,550,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-580)<=17){
				rect(100,580,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-620)<=17){
				rect(100,620,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-660)<=17){
				rect(100,660,100,34);
			}
			...
			if(accompvol==120){
				rect(100,190,105,40);   
			}else if (accompvol==80){
				rect(100,230,105,40);
			}else if(accompvol==40){
				rect(100,270,105,40);
			}else if(accompvol==0){
				rect(100,310,105,40);
			}
			...
			if( abs(mouseX-100)<=40 & abs(mouseY-190)<=17){
				rect(100,190,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-230)<=17){
				rect(100,230,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-270)<=17){
				rect(100,270,100,34);    
			}else if(abs(mouseX-100)<=40 & abs(mouseY-310)<=17){
				rect(100,310,100,34);
			}
	...
	public void mouseClicked() {
		if(isMenu){
			//メイン音量
			if( abs(mouseX-100)<=50 & abs(mouseY-540)<=17){
				mainvol=120;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-580)<=17){
				mainvol=80;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-620)<=17){
				mainvol=40;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-660)<=17){
				mainvol=0;
			} 
            //ハモリ上下
			if(abs(mouseX-100)<=50 & abs(mouseY-400)<=22){
			...
			//伴奏音量
			if( abs(mouseX-100)<=50 & abs(mouseY-190)<=17){
				accompvol=120;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-230)<=17){
				accompvol=80;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-270)<=17){
				accompvol=40;
			}else if(abs(mouseX-100)<=50 & abs(mouseY-310)<=17){
				accompvol=0;
			}  
...
class Diocomp extends SPModule {
	...
	public void execute(Object[] src, TimeSeriesCompatible[] dest) 
		...
		//calc hz->notenum
		//draw_kari = draw.drawf0;
		if(draw_kari == true){
			pitches.add((Math.log(median_f0/440) / Math.log(2)*12 )+69  + 12); //ここ周波数からノートナンバー
		}else{
			pitches.add(-100.0); //ENTERを押されない限り0.0を代入する
		}
	
		microseconds.add(All.cmx.getMicrosecondPosition());
		ticks.add(All.cmx.getTickPosition());

		//System.out.println(ticks.get(ticks.size()-1) + "   " + pitches.get(pitches.size()-1));
		//System.out.println(draw_kari);
...
class Diocomp extends SPModule {
	...
	public void execute(Object[] src, TimeSeriesCompatible[] dest) 
		...
		microseconds.add(HamoKara.cmx.getMicrosecondPosition());
		ticks.add(HamoKara.cmx.getTickPosition());
