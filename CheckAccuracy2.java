...
public class CheckAccuracy2 extends PApplet {
	...
	public void keyPressed() {
		...
		if(isMenu) {
			...
			if (keyCode == ENTER) {
				...
				midi = cmx.readSMFAsMIDIXML(createInput("../midis/X52612G2.MID"));        
				midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/44_re.mid"));    
				...
				partlist = scc.getPartList();
					...
					*/
					...
					for(int j=0;j<notelist.length;j++){
						for(int i=3;i>=0;i--){
							...
							if(plus){
								...
								//＋７
                                if((notenumData[j]+7)%12 == code_hamo[i][on_off]){
									//hamoData[j]=7;
								} 
							}else{
					...
					if(checking){
						for (int i=0; i<onsetData.length; i++) {
							for (int j=0;j<onsetCheck.length; j++){
								if (onsetData[i] == onsetCheck[j]  & offsetData[i] == offsetCheck[j]){
                                    tickMatch++;
									println("tickMatch:"+tickMatch);
									if(hamoNotenum[i] == notenumCheck[j]){
									    println("match:"+match);
										println("per:"+(double)match/musicLength);
										match++;
									}
								}
							}
						...
						try{
							File file = new File("./checkin/result_match2.txt");
		...
		void displayLyric( ) {
			...
			for (int i=0; i<2; i++) {
				if (lyriccount+i < lyrics.size()) {
                    ...
                    // draw F0
					for (int p = 1; p < CheckAccuracy2.dioM.ticks.size(); p++) {
						double x0 = (CheckAccuracy2.dioM.ticks.get(p-1) - div * (q-1)) * width / div;
						double x1 = (CheckAccuracy2.dioM.ticks.get(p) - div * (q-1)) * width / div;
						double y0 = linemid * (lineNum - ((CheckAccuracy2.dioM.pitches.get(p-1) + octave*12) - subst)-2);
						double y1 = linemid * (lineNum - ((CheckAccuracy2.dioM.pitches.get(p) + octave*12) - subst)-2);
