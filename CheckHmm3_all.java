...
public class CheckHmm3_all extends PApplet{
	...
	public void draw(){ 
		try{
			...
			try{
				File fileD = new File("./checkin/hmm_same.txt");
			...
			try{
				...
				File fileS = new File("./checkin/hmm_sameR.txt");
            ...
            try{
				File fileB = new File("./checkin/hmm_unizon.txt");
				FileWriter filewriter = new FileWriter(fileB,false); 

				File fileD =new File("./checkin/hmm_aij.txt");
				FileWriter filewriterD =new FileWriter(fileD,false); 

				File fileA = new File("./checkin/hmm_same_per.txt");
				FileWriter filewriterA = new FileWriter(fileA,false);

				File fileC = new File("./checkin/hmm_unizon_per.txt");
			...
			sameAij = new double[12][12];
			elseAij =new double[12][12];
            for(int i=0;i<12;i++){
				for(int j=0;j<12;j++){
					sameAij[i][j]=0;
					elseAij[i][j]=0;
			...
			for(File file : files){
				...
				}else if((file.toString()).contains("67_re")){midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00429G2.MID"));   
			   // }else if((file.toString()).contains("68_re")){midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00419G2.MID"));   
				...
				hamoSecondNote = new int[mainNotelist.length];
				for(int i=0; i<mainNotelist.length; i++){
					mainNote[i] = (int)mainNotelist[i].notenum();
					mainOnset[i] = mainNotelist[i].onset(scc.getDivision());
					mainOffset[i] = mainNotelist[i].offset(scc.getDivision());
				}
				...
				for(int i=0; i<mainOnset.length; i++){
					...
					if(keyChangeNum[countLoop2]<mainOnset[i] && countLoop2<keyChangeCount){
						...
						countLoop2++;
						println("NUM:"+keyChangeNum[countLoop2]+",mainL:"+mainOnset[mainOnset.length-1]);  
					}
					if(keyChangeNum[countLoop2]>=mainOnset[mainOnset.length-1]){
						keyChangeNum[countLoop2]=mainOnset.length-1;
						countLoop2++;
						//println("NUM:"+keyChangeNum[countLoop2]+",mainL:"+mainOnset.length);
					}
				}
				...
				for(int i=0 ;i< mainLength;i++){
					if(plus){
                        ...
                        if(mainNote[i]%12 <= hmmJ.hamosecond.get(i)){
							hamoSecondNote[i] = (mainNote[i]/12) * 12 +hmmJ.hamosecond.get(i);
						}else if(mainNote[i]%12 > hmmJ.hamosecond.get(i)){
							hamoSecondNote[i] = (mainNote[i]/12 + 1) * 12 +hmmJ.hamosecond.get(i);
						}   
					}else{
						if(mainNote[i]%12 < hmmJ.hamohamo.get(i)){
							hamoNote[i] = (mainNote[i]/12 -1) * 12 +hmmJ.hamohamo.get(i);
				...
				if(checking){
					...
					for (int i=0; i<mainLength; i++) {
						for (int j=0;j<onsetCheck.length; j++){
							...
							//println(tickMatch+"gggggggg"+match);
									match++;
								}else if(hamoSecondNote[i] == notenumCheck[j]){   
							        matchSecond++;
								}
							}
						}
					...
					try{
						...
						File fileC = new File("./checkin/hmm_collect.txt");
						FileWriter filewriterC = new FileWriter(fileC,true);


						File fileD =new File("./checkin/hmm_aij.txt");
						FileWriter filewriterD =new FileWriter(fileD,true);
						
						PrintWriter printwriter = new PrintWriter(new BufferedWriter(filewriter));
						PrintWriter printwriterC = new PrintWriter(new BufferedWriter(filewriterC));
						PrintWriter printwriterD = new PrintWriter(new BufferedWriter(filewriterD));
					    ...
					    printwriter.print("matchsecond="+matchSecond);
                        printwriter.println("match+matchsecond="+( match+matchSecond) 
								+ "per=" + (double)(match+matchSecond)/mainLength);  
						
						if((double)match/mainLength>0.6){
                            printwriterD.println("per=" + (double)match/mainLength); 
							printwriterD.println("aij");
							for(int i=0;i<12;i++){
								printwriterD.println(i+"===================");
								for(int j=0;j<12;j++){

									printwriterD.println(aij[i][j]);
								}
								printwriterD.println("");
							}
						}
						printwriterD.println("=================");
						printwriterD.close();
						...
						printwriter.println("=====================");
						printwriterC.println((double)match/mainLength);
						printwriterC.close();
						printwriter.close();
					}catch(IOException e){
						System.out.println(e);
   				...
   				try{
						File fileC = new File("./checkin/hmm_same.txt");
						FileWriter filewriterC = new FileWriter(fileC,true);
						File fileS = new File("./checkin/hmm_sameR.txt");
						...
						PrintWriter printwriterA = new PrintWriter(new BufferedWriter(filewriterA));          
						beforeNote = (int)hamoNote[0];
						samenum=0;
						maxSamenum=0;
						sameCount=0;
						//beforeMainNote = (int)mainNote[0];
beforeMainNote = (int)notenumCheck[0];           
						samenumMain=0;
						maxSamenumMain=0;
						sameCountMain=0;
						beforeSame=0;
						...
						for(int i=1;i<mainLength;i++){
							...
							beforeMainNote=(int)notenumCheck[i-1];
							currentMainNote=(int)notenumCheck[i]; 
							...
							if(beforeNote!=currentNote){ 
								...
								if(beforeSame>=10){
									printwriterA.println("ss"+beforeSame);
									for(int j=i-beforeSame;j<i;j++){
										per_same_all++;
										//printwriterA.println("hamoNote["+j+"]:"+hamoNote[j]+"notenumCheck["+j+"]"+notenumCheck[j]); 
										//printwriterA.println("On["+j+"]:"+mainOnset[j]+"off["+j+"]"+mainOffset[j]);
                                        //printwriterA.println("On["+j+"]:"+onsetCheck[j]+"off["+j+"]"+offsetCheck[j]); 
										for (int q=0;q<onsetCheck.length; q++){  
											if (mainOnset[j] == onsetCheck[q]  & mainOffset[j] == offsetCheck[q]){

												if(hamoNote[j]==notenumCheck[q]){
													//printwriterA.println("hamoNote["+j+"]:"+hamoNote[j]+"notenumCheck["+j+"]"+notenumCheck[j]);
													mnum[hamoNote[j]%12]++;
													per_same++; 
						...
						if(maxSamenum>=10){
							musicCount++;
							printwriterA.println("max:"+maxSamenum);
							printwriterA.println(file);

							printwriterA.println("per_same_all:"+per_same_all);
							printwriterA.println("same_per:"+per_same);
							if(per_same_all!=0){
								printwriterA.println("SAMEper:"+(double)per_same/per_same_all);
							}else{printwriterA.println("SAMEper:!");}
							printwriterA.println("per:"+(double)match/mainLength);
							//printwriterA.println("mnum");
							for(int i=0;i<12;i++){
							printwriterA.print(i+":"+mnum[i]);
							}
							printwriterA.println("");
							for(int i=0;i<12;i++){
								for(int j=0;j<12;j++){
									if(Double.isNaN(aij[i][j])){
										aij[i][j]=0;
									}
                                    sameAij[i][j]+=aij[i][j];
									//printwriterA.println(aij[i][j]);
								}
							}
								printwriterA.println("");
						  
							//printwriterA.println("check"+check);
							printwriterA.close();
						}else{ 
							musicCountElse++; 
							for(int i=0;i<12;i++){
				...
				try{
					...
					File fileC = new File("./checkin/hmm_unizon_collect.txt");
					FileWriter filewriterC = new FileWriter(fileC,true);  

					File fileD = new File("./checkin/hmm_unizon_only.txt");
					FileWriter filewriterD = new FileWriter(fileD,true);  
					...
					for(int i=0;i<mainLength;i++){
						...
						}else if(mainNote[i]==notenumCheck[i]){
							sameUnizonCollect++;
						}
					}
					printwriter.println((double)sameUnizon/mainLength);
					printwriterC.println((double)sameUnizonCollect/mainLength);  
					printwriterD.println((double)onlyUnizon/sameUnizon);  
					printwriter.close();
					printwriterC.close();     
					printwriterD.close();   
				}catch(IOException e){
					...
					try{
						File fileB = new File("./checkin/hmm_same5.txt");
						FileWriter filewriter = new FileWriter(fileB,true);
                      
						PrintWriter printwriter = new PrintWriter(new BufferedWriter(filewriter)); 

						File fileC = new File("./checkin/hmm_collect5.txt");
						FileWriter filewriterC = new FileWriter(fileC,true);
                      
						PrintWriter printwriterC = new PrintWriter(new BufferedWriter(filewriterC));
						//if(maxSamenum>=5){
							printwriter.println(maxSamenum);
							printwriterC.println((double)match/mainLength);
						//}
						printwriter.close();
			...
			try{
				File fileH = new File("./checkin/hmm_same_hist.txt");
				...
				File fileA = new File("./checkin/hmm_aij_same.txt");
				FileWriter filewriterA = new FileWriter(fileA,true);
				File fileB = new File("./checkin/hmm_aij_else.txt");
				...
				PrintWriter printwriterA = new PrintWriter(new BufferedWriter(filewriterA));               
				PrintWriter printwriterB = new PrintWriter(new BufferedWriter(filewriterB)); 
				for(int i=0;i<50;i++){
				...
				for(int i=0;i<12;i++){
					printwriterA.println("==========");
					printwriterB.println("==========");  
					for(int j=0;j<12;j++){
						printwriterA.println(sameAij[i][j]/musicCount);
						printwriterB.println(elseAij[i][j]/musicCountElse);
					}
				}
				println("musicCount:"+musicCount);
				println("musicCountElse:"+musicCountElse);
...
class Jahmm {
	...
	public static ArrayList<Integer> hamosecond= new ArrayList<Integer>(); 
	...
	public static void main(int[] mainNote,double[][] code,int mainLength,int[] keyChangeNum,int[] keyChange,String[] keyChangeMode,boolean plus,double[][] aij) {
		...
		for(int j=0;j<keyChangeNum.length-1;j++){
			...
			for (int i = 0; i < states.length; i++){
			    ...
			    hamosecond.add(viterbi.secondaryState(i));
