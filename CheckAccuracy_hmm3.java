...
public class CheckAccuracy_hmm3 extends PApplet {
		...
		if(isMenu) {
			...
			if (keyCode == ENTER) {
				...
				if(musicnum==0){
					...
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/01_re.mid"));    
				}else if(musicnum==11){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X52644G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/02_re.mid"));    
				}else if(musicnum==12){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X55348G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/03_re.mid")); 
				}else if(musicnum==13){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00161G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/04_re.mid"));    
				/*}else if(musicnum==14){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X25943G.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/05_re.mid")); */

				}else if(musicnum==15){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X25943G.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/06_re.mid")); 
				}else if(musicnum==16){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X10675G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/07_re.mid")); 
				}else if(musicnum==17){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X26451G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/08_re.mid")); 
				}else if(musicnum==18){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X52191G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/09_re.mid")); 
				}else if(musicnum==19){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X53846G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/10_re.mid")); 
				}else if(musicnum==20){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X54664G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/11_re.mid")); 
				}else if(musicnum==21){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00512G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/12_re.mid")); 
				}else if(musicnum==22){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X29048G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/13_re.mid")); 
				}else if(musicnum==23){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X46920G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/14_re.mid")); 
				}else if(musicnum==24){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X54842G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/15_re.mid")); 
				}else if(musicnum==25){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X54963G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/16_re.mid")); 
				}else if(musicnum==26){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X55654G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/17_re.mid")); 
				}else if(musicnum==27){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/XG5636K1.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/18_re.mid")); 
				}else if(musicnum==28){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X48018G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/19_re.mid")); 
				}else if(musicnum==29){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/XG5506G.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/20_re.mid")); 
				}else if(musicnum==30){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/XG5415G.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/21_re.mid")); 

				}else if(musicnum==38){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X52862G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/22_re.mid"));     
				}else if(musicnum==31){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X54900G2.MID"));         
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/23_re.mid")); 
				}else if(musicnum==32){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X53199G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/24_re.mid"));
				}else if(musicnum==33){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X55627G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/25_re.mid"));    
				}else if(musicnum==34){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00088G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/26_re.mid"));    
				}else if(musicnum==35){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00457G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/27_re.mid"));    
				}else if(musicnum==36){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X22496G.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/28_re.mid"));    
				}else if(musicnum==37){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/X00377G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/29_re.mid"));    
				}else if(musicnum==38){
					midi = cmx.readSMFAsMIDIXML(createInput("../midis/R00327G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/30_re.mid")); 
				}else if(musicnum==100){
				    midi = cmx.readSMFAsMIDIXML(createInput("../midis/X25869G2.MID"));        
					midiCheck = cmx.readSMFAsMIDIXML(createInput("./checkin/midi/37_re.mid"));          
				...
				try {
					...
					SCCDataSet.Part[] partsHamo = scc2.toDataSet().getPartList();         
					for(SCCDataSet.Part p:partsHamo){
						if(p.channel()==15){
							scc2.removePart(p);
							break;
						}
					...
					println("keyChangeCount:"+keyChangeCount);
					...
					for(int i=0;i<12;i++){
                     ...
                     try{
						File fileB = new File("./checkin/hmm_08_only.txt");
					...
					//伴奏音量変更    
                    //伴奏うるさい
					SCCDataSet.Part[] parts_all = scc2.toDataSet().getPartList();              
					if(isAccompany){ 
						//parts = scc2.toDataSet().getPartList();         
						SCCDataSet.Part part_all = null;
						for (int i = 0; i < parts_all.length; i++) {
							if (parts_all[i].channel() != 1 && parts_all[i].channel()!=15) {
								part_all = parts_all[i];
								//println("kaakakk");
								...
								MutableMusicEvent[] events_all = part_all.getNoteList(); 
								events_all = part_all.getNoteList();     
								for (MutableMusicEvent evt : events_all) {
									if (evt instanceof MutableProgramChange) {
										//((MutableProgramChange)evt).setValue(4);       //音色変更
									} else if (evt instanceof MutableControlChange) {
										MutableControlChange c = (MutableControlChange)evt;
										c.setCtrlNum(7);                     //ボリューム変更
										c.setValue(accompvol);
									}
								}   
							}
		...
		void displayLyric( ) {
			...
			for (int i=0; i<2; i++) {
				if (lyriccount+i < lyrics.size()) {
                    ...
                    // draw F0
					for (int p = 1; p < CheckAccuracy_hmm3.dioM.ticks.size(); p++) {
						double x0 = (CheckAccuracy_hmm3.dioM.ticks.get(p-1) - div * (q-1)) * width / div;
						double x1 = (CheckAccuracy_hmm3.dioM.ticks.get(p) - div * (q-1)) * width / div;
						double y0 = linemid * (lineNum - ((CheckAccuracy_hmm3.dioM.pitches.get(p-1) + octave*12) - subst)-2);
						double y1 = linemid * (lineNum - ((CheckAccuracy_hmm3.dioM.pitches.get(p) + octave*12) - subst)-2);
...
class Jahmm {
	...
	public static void main(int[] mainNote,double[][] code,int mainLength,int[] keyChangeNum,int[] keyChange,String[] keyChangeMode,boolean plus,double[][] aij) {
		...
		for(int j=0;j<keyChangeNum.length-1;j++){
			...
			List<ObservationInteger> oseq1 = new ArrayList<ObservationInteger>();            System.out.println("MODE"+keyChangeMode[j]);    
